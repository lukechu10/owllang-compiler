...
fn can_parse_function() {
    ...
    assert_eq!(
        format!("{:?}", ast),
        r#"CompilationUnit { entry_file_name: "entry", functions: [Stmt { kind: Fn { proto: FnProto { args: [], iden: "f" }, body: Some(Stmt { kind: Block { statements: [Stmt { kind: Return { value: Expr { kind: Literal(0) } } }] } }) } }] }"#
    );
}

...
fn can_create_sourcefile() {
    ...
    assert_eq!(source.newline_pos, vec![
        BytePos(0),
        BytePos(6),
        BytePos(12),
        BytePos(18)
    ...
    assert_eq!(source.lookup_line(BytePos(5)).unwrap(), 0, "newline character should be on previous line");
    assert_eq!(source.lookup_line(BytePos(6)).unwrap(), 1);
    assert_eq!(source.lookup_line(BytePos(100)), None, "out of bounds should return None");
}